...
filter ::
  ...
  -> List a
filter f =
  foldRight (\a xs -> if f a then a :. xs else xs) Nil
...
-- | Flatten a list of lists to a linst.
--
...
-- notReverse (1 :. Nil) ++ notReverse (2 :. Nil)
-- 1 :. 2 :. Nil

-- notReverse ((2 :. Nil) ++ (1 :. Nil))
-- 1 :. 2 :. Nil
